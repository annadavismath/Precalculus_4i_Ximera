...
\begin{definition}  A \textbf{polynomial}\index{polynomial} is a sum of terms each of which is a real number or a real number multiplied by one or more variables to natural number powers. 
\end{definition}
...
\begin{definition}\label{polynomialterminology} \textbf{Polynomial Vocabulary}
...
\begin{theorem}\label{generaldistprop} \textbf{Generalized Distributive Property:}  To multiply a quantity of $n$ terms by a quantity of $m$ terms, multiply each of the $n$ terms of the first quantity by each of the $m$ terms in the second quantity and add the resulting $n \cdot m$ terms together. 
...
\begin{theorem}\label{FOIL} \textbf{F.O.I.L:} The terms generated from the product of two binomials: $(a + b)(c+d)$ can be verbalized as follows: ``Take the sum of
...
\begin{theorem}\label{SpecialProducts} \textbf{Special Products:} Let $a$ and $b$ be real numbers:
...
\begin{theorem} \label{polydiv} \textbf{Polynomial Division:}  Let $d$ and $p$ be nonzero polynomials where the degree of $p$ is greater than or equal to the degree of $d$.  There exist two unique polynomials, $q$ and $r$, such that $p = d \cdot q + r,\,$ where either $r = 0$ or the degree of $r$ is strictly less than the degree of $d$.
\end{theorem}
%% \ebm}
...
%% SKIPPED %% \input{ExercisesforAppPolyArith}
